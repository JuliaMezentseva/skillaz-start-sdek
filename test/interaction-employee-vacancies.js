...
(async () => {
  try {
    ...
    // Отметка "Вы откликнулись" — есть отклик на v3 (Продуктовый аналитик) по данным myResponses
    console.log("Отметка 'Вы откликнулись' на вакансии с уже отправленным откликом:", /Продуктовый аналитик[\s\S]{0,400}Вы откликнулись/.test(body) || /Вы откликнулись[\s\S]{0,400}Продуктовый аналитик/.test(body) ? "PASS" : "FAIL");
